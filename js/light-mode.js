...
formLogIn.addEventListener("submit" , (event) => {
    ...
    accLs++
    localStorage.setItem("Usuario Numero" , accLs)
...
/*
let acc = 0 //ACUMULADOR PARA SABER EL NUMERO DE USUARIOS
...
formLogIn.addEventListener("submit" , (event) => {
    ...
    newUser[acc] =  { ////GUARDAMOS EL VALUE EN UN OBJETO DE TIPO USUARIO EN LA POSICION 'acc' QUE INDICA EL NUMERO DE USUARIO
        Email: inpEmail,
        Usuario: inpUsuario,
        Password: inpPassword
    }
    ...
    if(newUser[acc].Email.trim() !== "" && newUser[acc].Usuario.trim() !== "" && newUser[acc].Password.trim() !== ""){ //VERIFICAMOS QUE NO SEA NULL
        userJSON[acc] = JSON.stringify(newUser[acc]) //PASAMOS EL OBJETO A JSON EN LA POSICION 'acc' QUE INDICA EL NUMERO DE USUARIO
        localStorage.setItem("Usuario " + acc , userJSON[acc]) //SUBIMOS A LOCAL STORAGE EN FORMATO JSON
    } 
